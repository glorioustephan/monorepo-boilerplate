...
4. Add `src/index.ts` with a real export. For React packages, add an `.oxlintrc.json`
   extending `../../tooling/oxc-config/react.json`.
5. If it has local conventions, add a short `AGENTS.md` + a one-line `CLAUDE.md` (`@AGENTS.md`).
6. Run `pnpm install`, then `pnpm typecheck && pnpm lint` and fix anything that fails.
